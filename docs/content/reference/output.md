...
describes what users get. Pick a format with `-o`, or let swpc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
swpc <command> -o table     # a rounded, color-aware grid for reading
swpc <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
swpc <command> -o list      # one record per section, easy on the eyes
swpc <command> -o jsonl     # one JSON object per line, for piping
swpc <command> -o json      # a single JSON array
swpc <command> -o csv       # spreadsheet friendly
swpc <command> -o tsv       # tab-separated
swpc <command> -o url       # just the URL column
swpc <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
swpc <command> --fields id,title,url
```
...
```bash
swpc <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
swpc <command>            # a table, because this is a terminal
swpc <command> | wc -l    # JSONL, because this is a pipe
```
